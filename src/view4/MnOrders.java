...
public class MnOrders extends JPanel {
	...
	private String[] filterTitle = { "Theo mã đơn hàng", "Theo tên khách hàng" };
	...
	private String[] filterDate = { "Theo ngày lập", "Theo ngày giao" };
	private JComboBox jcbD = new JComboBox(filterDate);

	private JLabel lbSince = new JLabel("Từ ngày ");
	private JTextField jtfSince = new JTextField();
	private JButton btPickSince = new JButton("");

	private JDateChooser chooseSince = new JDateChooser();

	private JLabel lbUntil = new JLabel("Đến ngày ");
	private JTextField jtfUntil = new JTextField();
	private JButton btPickUntil = new JButton("");
	...
	private float[] columnWidthPercentage = { 0.03f, 0.06f, 0.1f, 0.16f, 0.12f, 0.18f, 0.1f, 0.1f, 0.1f, 0.1f };
	double rateWidthTable = 1.0;

	public MnOrders(String[] columnNames, Object[][] rowData) {
		...
		panelBtns3.add(new JLabel("  Thao tác bán hàng"), BorderLayout.NORTH);
...
//		TableCellRenderer tableRenderer = jTable.getDefaultRenderer(JButton.class);
//		jTable.setDefaultRenderer(JButton.class, new JTableButtonRenderer(tableRenderer));
		new ButtonColumn1(jTable, 1);
...
//			else if (x == 1) {
//				TableColumn detail = jTable.getColumnModel().getColumn(x);
//				detail.setCellRenderer(centerRenderer);
//			}
		}
